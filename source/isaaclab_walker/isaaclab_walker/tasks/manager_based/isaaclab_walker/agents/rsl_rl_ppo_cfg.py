...
# --- TargetCfg (155D, history_length=1) ---
...
# + command_delay(1, normalized) + motor_strength(13, [0.8,1.2])
_TARGET_OBS_SCALE: list[float] = (
    ...
    + [1.0] * 13 # motor_strength (per-joint, [0.8, 1.2])
)
assert len(_TARGET_OBS_SCALE) == 155, f"target scale must be 155D, got {len(_TARGET_OBS_SCALE)}"

# --- CriticCfg (161D, history_length=1) ---
...
assert len(_CRITIC_OBS_SCALE) == 161, f"critic scale must be 161D, got {len(_CRITIC_OBS_SCALE)}"
...
class P73RoughPPORunnerFutureCfg(RslRlOnPolicyRunnerCfg):
    ...
    #   + push_force(3) + command_delay(1) + motor_strength(13) = 155D
