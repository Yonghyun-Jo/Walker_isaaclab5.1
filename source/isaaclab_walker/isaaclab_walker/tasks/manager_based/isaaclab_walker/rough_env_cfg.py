...
class KangarooRewards(RewardsCfg):
    ...
    joint_deviation_right_hiproll = RewTerm(
        ...
        params={
            ...
            "deadband_neg": 0.3,   # 벌림 허용
            "stiffness_pos": 2.5,
            "stiffness_neg": 1.5,
        },
    )

    # Knee: Walker L/R both -y axis (same axis), range [-2.56, 0], default = -0.76
    # 같은 축 → L/R 같은 deviation 설정
    # q_target = default(-0.76) + offset(-0.15) = -0.91
    # e > 0 (pos): toward extension(과신전) → 강하게 제한
    # e < 0 (neg): deeper flexion(굴곡) → 넓게 허용
...
class ActionsCfg:
    joint_pos = mdp.LowerBodyActionsCfg(
        ...
        p_gains=[5000.0, 4000.0, 2000.0, 5000.0, 4000.0, 4000.0,
                 5000.0, 4000.0, 2000.0, 5000.0, 4000.0, 4000.0,
                 6000.0],
